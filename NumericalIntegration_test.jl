...
module NumericalIntegration_test

using Compat

export integrate_test
...
function integrate_test{X<:Number, Y<:Number}(x::AbstractVector{X}, y::AbstractVector{Y}, ::Trapezoidal)
...
function integrate_test{X<:Number, Y<:Number}(x::AbstractVector{X}, y::AbstractVector{Y}, ::TrapezoidalEven)
    @assert length(x) == length(y) "x and y vectors must be of the same length!"
    return 0.5 * (x[end] - x[1]) / (length(y) - 1) * (y[1] + y[end] + sum(y[2:end-1]))
end

function integrate_test{X<:Number, Y<:Number}(x::AbstractVector{X}, y::AbstractVector{Y}, ::TrapezoidalFast)
    retval = zero(promote(x[1], y[1])[1])
...
function integrate_test{X<:Number, Y<:Number}(x::AbstractVector{X}, y::AbstractVector{Y}, ::TrapezoidalEvenFast)
    retval = zero(promote(x[1], y[1])[1])
    N = length(y) - 1
...
function integrate_test{X<:Number, Y<:Number}(x::AbstractVector{X}, y::AbstractVector{Y}, ::SimpsonEven)
    @assert length(x) == length(y) "x and y vectors must be of the same length!"
...
function integrate_test{X<:Number, Y<:Number}(x::AbstractVector{X}, y::AbstractVector{Y}, ::SimpsonEvenFast)
...
integrate_test{X<:Number, Y<:Number}(x::AbstractVector{X}, y::AbstractVector{Y}) = integrate_test(x, y, TrapezoidalFast())
